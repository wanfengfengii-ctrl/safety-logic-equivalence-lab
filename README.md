...
- 节点数设有上限（`MAX_BDD_NODES`），超限抛出 `BddLimitError`，防止中间 BDD 异常膨胀。

### 满足赋值摘要不变量（核心）

每个唯一化内部节点在 `mk` 时同步维护一个到 1 终端的**最小满足赋值摘要** `sat: Uint8Array`：

- 终端 1 摘要为全 0（函数已被满足）；
- 节点规则：**低分支可满足（`low !== 0`）时取低分支**，本变量位置写 0、复制 `sat(low)`；
  **否则取高分支**，本变量写 1、复制 `sat(high)`；
- 子树中被跳过（R1 消去或本就不判定）的变量一律保持 **0**。

因为摘要只在 `mk` 中随 R1/R2 一起产生与复用，Apply 与否定的结果全部来自 `mk`，
所以该不变量在建点与任何约简后都保持成立（单元测试对全部合成节点逐一核验）。

### 等价判定与反例

1. 分别按 DAG 记忆化递归把两图编译为同一管理器内的 ROBDD（共享唯一表）；
